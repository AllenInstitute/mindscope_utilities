...
def event_triggered_response(data, t, y, event_timestamps, t_before=1, t_after=1, output_sampling_rate=10, include_endpoint=True, output_format='tidy'):  # NOQA E501
    ...
    '''
    # set up a dictionary with key 'timestamps' and
    ...
    if output_format == 'wide':
        ...
        return wide_etr.set_index('timestamps')
    elif output_format == 'tidy':
        # if output format is 'tidy',
        # transform the wide dataframe to tidy format
        # first, melt the dataframe with the 'id_vars' column as "timestamps"
        tidy_etr = wide_etr.melt(id_vars='timestamps')
        ...
        # add an "event_timestamps" column that contains the event timestamps ()
        tidy_etr['event_timestamps'] = tidy_etr['variable'].map(
